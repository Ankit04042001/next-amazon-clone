...
function CartProduct({ item }: cartProductProps) {
    ...
    return (
        <div className='p-4 bg-gray-100 rounded-lg flex items-center gap-4 flex-col md:flex-row'>
            <Image className='object-contain w-20 h-20 md:w-[150px] md:h-[150px]' width={150} height={150} src={item.image} alt='productImage' />
            <div className="w-full flex items-center justify-between px-2 gap-4">
                <div className="flex flex-col gap-1">
                    <p className="text-sm md:text-lg font-semibold text-amazon_blue">{item.title}</p>
                    ...
                    <p className="text-sm md:hidden font-semibold">
                        Subtotal{" "}
                        <span className="font-semibold text-amazon_blue">
