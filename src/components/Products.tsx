    ...
    <div className='w-full px-6 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6'>
      {
        productData.map((
          ...
          <div key={id} className='w-full bg-white text-black p-4 border-gray-300 rounded-lg group overflow-hidden relative'>
            ...
            <hr />
              <div className='px-1 py-3 flex flex-col gap-1 grow mb-8'>
              ...
              <button onClick={()=>dispatch(addToCart({
                ...
                title:title,
                quantity:1
              }))} className='w-[90%] h-10 font-medium bg-amazon_blue text-white rounded-md hover:bg-amazon_yellow hover:text-black duration-300 absolute bottom-2'>add to cart</button>
            </div>
        ))
      }
    </div>
